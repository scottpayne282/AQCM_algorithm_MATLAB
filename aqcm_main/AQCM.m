function [ClustCell,InfoCell,n]=AQCM(S,tol,lam,tee)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [A]=contractionstep(F,N,S)
...
if length(ind)<2
    ...
    for i=1:numinds% need end
        sC = sum( F(:,I(i)) & F(:,J(i)) );
        sAnB = sum( F(:,I(i)) & ~F(:,J(i)) );
        sBnA = sum( ~F(:,I(i)) & F(:,J(i)) );
        A(I(i),J(i)) =(  sum(sum(  S( F(:,I(i)) , F(:,J(i)) )  )) ...
   -0.5*( sum(sum( S( F(:,I(i)) & F(:,J(i)) , F(:,I(i)) & F(:,J(i)) ) )) )  )...
   / ...     
(  sC*(sC-2)/2 + sC*(sAnB + sBnA) + sAnB*sBnA  ); 
    end
