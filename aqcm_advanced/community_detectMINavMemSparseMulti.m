function [T,TCell,CommunityCell]=community_detectMINavMemSparseMulti(...
    ClustCell,InfoCell,n,x)

% x is the number of clustering outputs we want to get
TCell = cell(1,x);
%-------------------------------------------------------------------------
...
for ii = 1:x
    ...
    TCell{ii}=sort(Tlabels);
    locs = ismember(E(:,3),Tlabels')';
    E(locs,1) = Inf;
end
%--------------------------------------------------------------------------
%--------------------------------------------------------------------------
...
% 
CommunityCell = cell(x,1);
for ii = 1:x
    [Comms]=leaves_from_tree_nodes_inside(T,n,TCell{ii});
    CommunityCell{ii} = Comms;
end
%--------------------------------------------------------------------------
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
